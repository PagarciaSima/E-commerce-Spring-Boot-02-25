...
public class CartService {
     ...
     * @throws RuntimeException If the product with the given {@code productId} is not found.
     */
    @Transactional
    ...
    /**
     * Retrieves paginated and sorted cart details for the authenticated user, optionally filtered by a search key.
     *
     * @param page      The page number (starting from 0).
     * @param size      The number of items per page.
     * @param searchKey The search keyword for filtering cart items by product name (optional).
    ...
    public PageResponseDto<CartEntity> getCartDetailsOrderedByNameWithPagination(int page, int size, String searchKey) {
        UserEntity user = getAuthenticatedUser();
        log.info("Recuperando carrito paginado para usuario: {}, página: {}, tamaño: {}, búsqueda: {}", user.getUserName(), page, size, searchKey);
        ...
        if (searchKey == null || searchKey.trim().isEmpty()) {
            ...
            cartPage = this.cartDao.findByUserEntityAndProductEntityProductNameContainingIgnoreCase(user, searchKey, pageable);
        }

        return new PageResponseDto<>(
            cartPage.getContent(), // Lista de elementos (CartEntity)
